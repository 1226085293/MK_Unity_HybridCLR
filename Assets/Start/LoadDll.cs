...
public class LoadDll : MonoBehaviour
{
    ...
    void LoadGameDll()
    {
        ...
        {
            aotDllBytes[i] = Addressables.LoadAssetAsync<TextAsset>("Assets/Dlls/" + aotDlls[i] + ".bytes").WaitForCompletion();
        }
        TextAsset hotfixDll = Addressables.LoadAssetAsync<TextAsset>("Assets/Dlls/Main.dll.bytes").WaitForCompletion();
        gameAss = Assembly.Load(hotfixDll.bytes);
#else
        gameAss = AppDomain.CurrentDomain.GetAssemblies().First(assembly => assembly.GetName().Name == "Main");
#endif
    }
